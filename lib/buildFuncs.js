...
export default class BuildFuncs {
    ...
    constructor(config, clean = false) {
        ...
        if (config.buildMode === 'production') {
            this.sassConfig.style = 'compressed'
        } else {
            this.sassConfig.sourceMap = true
            this.sassConfig.sourceMapIncludeSources = true
        }
    ...
    createBrowsersync(options) {
        ...
        if (this.config.watchMode === 'standalone') {
            const useHttps = this.config.standalone.key && this.config.standalone.cert
            console.log(this.config.standalone)
            const https = {
                key: this.config.standalone.key,
                cert: this.config.standalone.cert
            }
            browserSyncOptions = {
                open: 'external',
                ...
                watch: true,
                server: {
                    baseDir: path.join(this.config.getPackagesDir(), this.config.standalone.package, 'Resources/Public/Standalone'),
                    directory: true,
                ...
                notify: notify,
                files: [
                    path.resolve(this.config.getPackagesDir(), this.config.standalone.package, 'Resources/Public/Standalone', '*')
                ]
            }
        } else {
            const useHttps = this.config.typo3.key && this.config.typo3.cert
            const https = {
                key: this.config.typo3.key,
                cert: this.config.typo3.cert
            }
            browserSyncOptions = {
                open: false,
                port: this.config.typo3.port,
                host: this.config.typo3.host,
                proxy: {
                    target: this.config.proxy,
                    proxyReq: [
                        function(proxyReq) {
                            const host = proxyReq.getHeaders()['x-forwarded-host'].split(':')
                            proxyReq.setHeader('host', host[0])
                        }
                    ]
                },
                ui: false,
