    ...
    createBrowsersync() {
        this.bs = new Bs.init({
            ...
            files: [
                {
                    match: [
                        path.join(this.config.getPackagesDir(), 'typo3temp/ReloadFrontend.now')
                    ],
                    fn: (event, file) => {
                        log.info(new Date.now().toLocaleString(), 'clearCache by Backend')
                        this.bs.reload()
                    }
                },
                {
                    ...
                    fn: (event, file) => {
                        log.info(new Date.now().toLocaleString(), 'clearCache')
                        exec(clearCacheCmd, () => {
                            this.bs.reload()
                        })
                    }
                },
                {
                    ...
                    fn: (event, file) => {
                        log.info(new Date.now().toLocaleString(), 'clearAllCache')
